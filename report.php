...
?>
    ...
    <div style="padding-top:20px;" align="center">
        <div class = "heading" >
            ...
            <label style="" class="headers">Итоговый отчет о работе Центра консультационной поддержки</label>
        ...
        <ul class="menu">
            ...
            <li class="current"><a href="report.php" >Итоговая форма</a></li>
    ...
    <form method="post" action="search.php" id="formMain" name="formMain" class="formreg" enctype="multipart/form-data"
        ...
        <table width="500px" border="0" cellspacing="0" cellpadding="0" id="bdr_w" align="center">
            ...
            <tr valign="top">
                ...
                <td class="bt1" width="600px">
                    <input type="text" class="fld" id="monthTodayTo" value="" name="dateto">
                </td>
            </tr>
            ...
            <tr valign="top">
                ...
                <td class="bt1">
                    <select name="ckp[]" id="ckp" class="fld" multiple="multiple" size=5 style="height: 80px;">
                        ...
                        <?php
                        $result = mysql_query("SELECT City, idCCS, b.District_idDistrict, c.Branch_idBranch,b.idBranch FROM ccs c
                  INNER JOIN branch b ON c.Branch_idBranch = b.idBranch
                INNER JOIN district d ON b.District_idDistrict = d.idDistrict ORDER BY c.Name")
                        or die ("<b>Ошибка в обращении к БД. Невозможно выполнить запрос.");
                        while ($row = mysql_fetch_array($result)) {

                            echo '<option data-branch ="'. $row['idBranch'] .'"  data-districtId ="'. $row['District_idDistrict'] .'"
                             data-id_ckp ="'. $row['idCCS'] .'" data-id_branch="' . $row['Branch_idBranch'] . '"
            ...
            <tr valign="top">
                ...
                <td class="bt1">
                    <select name="manager[]" id="manager" class="fld" multiple="multiple" size=5 style="height: 80px;">
                        ...
                        $result = mysql_query("SELECT m.Name, m.Login, m.idManager, b.idBranch,b.District_idDistrict,m.Branch_idBranch
                                          FROM manager m
                        ...
                        while ($row = mysql_fetch_array($result)) {
                            ...
                            echo '<option data-idDistrict = "' . $row['District_idDistrict'] . '" data-manager ="' . $row['Branch_idBranch'] . '" value="' . $row['idManager'] . '">' . $row['Name'] . '</option>';
                        }
                        ?>
            ...
            <tr>
                <td colspan="5" align="center">
                    ...
                    <input id="add_data" type="button" name="add_data" data-submitted="false"
                           value="Добавить данные" onclick="$('#hidden').val('true'); location.href='add.php'"/>
                </td>
            </tr>
            </tbody>
        </table>
    ...
    <script>
        $(function() {
            var $dis = $('#ckp').children().show();
            var $dis = $('#manager').children().show();
        });
        ...
        $('select#district').change(function () {
            ...
            if ( $d == 0){
                //$('#branch').children().show();
                $('#ckp').children().show();
               $('#manager').children().show();
            }else {
...
//            var $dis = $('#manager').children().show();
                ...
                var $ckp = $('#ckp').children().hide(),
                    districtCollection = $(this).val();
                $.each(districtCollection, function(key, item) {
                    $ckp.filter('[data-districtId='+ item +']').show();
                });
                var $manager = $('#manager').children().hide(),
                    districtCollection = $(this).val();
                $.each(districtCollection, function(key, item) {
                    $manager.filter('[data-idDistrict='+ item +']').show();
                });
                //var $branch = $('#branch').children().show();
//            var $dis = $('#ckp').children().show();
//            var $dis = $('#manager').children().show();
            }
        });
        ...
        $('select#ckp').change(function () {
            //var $managers = $('#manager').children().show();
            var $branch = $('#branch').children().show();
            var $dis = $('#district').children().show();
        ...
        $('select#ckp').change(function(){
            var ckp = $(this).children(':selected').attr('data-branch'),
                $managers = $('#manager').children().show();
            $managers.each(function(){
                var $manager = $(this),
                    branch_manager_id = $manager.attr('data-manager') || ckp;
                if (branch_manager_id != ckp) {
                    $manager.hide();
                }
            });
        });
...
//        $('select#ckp').change(function () {
//            var $manager = $('#manager').children().hide(),
//                managerCollection = $(this).attr('data-id_branch');
//            $.each(managerCollection, function(key, item) {
//                $manager.filter('[data-manager='+ item +']').show().select();
//            });
//
//        });
