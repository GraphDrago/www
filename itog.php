           ...
           // $month = $_POST['date'];
            ...
            if($month != ''){
                ...
                $sql_date = mysql_query("Select CONVERT( Date,char(10)) as Date, idFacts From fact
                                                     Where CCS_idCCS = $idckp and Date = '".$month."'")
...
<td><input type='button' id = 'file_button' value='Удалить' onclick=\"if(confirm('Вы действительно хотите удалить файл?'))
                    ...
                    if($i4 == 0)
                    {
                        $val4 = mysql_query("SELECT Date,CountPresentationWithOpening, idFacts
                       FROM fact f
                       INNER JOIN ccs c ON f.CCS_idCCS = c.idCCS
                       INNER JOIN manager m ON f.Manager_idManager = m.idManager
                       INNER JOIN branch b ON c.Branch_idBranch = b.idBranch
                     ...
                     Невозможно выполнить запрос.Попробуйте обновить страницу или обратится к администратору.'
                        ...
                        if($num_rows =  mysql_num_rows($val4) != 0) {
                            ...
                            $i4 = $it['CountPresentationWithOpening'];
                        }else{
                            $i4 =0;
                        }
                    }
                    ...
                    if($i1 == 0)
                    {
                       ...
                       INNER JOIN branch b ON c.Branch_idBranch = b.idBranch
                       INNER JOIN district d ON b.District_idDistrict = d.idDistrict
                       WHERE   c.CCS_idCCS = $idckp and CountCallWithOpening != 0 and  Date > '2015-01'
                       ORDER BY Date DESC LIMIT 1")
                        or die (json_encode(array('res' => 'Ошибка при обращении к БД.
                     Невозможно выполнить запрос.Попробуйте обновить страницу или обратится к администратору.'
                        )));
                        if($num_rows =  mysql_num_rows($val1) != 0) {
                            $it1 = mysql_fetch_assoc($val1);
                            $i1 = $it1['CountCallWithOpening'];
                        }else{
                            $i1 =0;
                        }
                    ...
                    if($i2 == 0)
                    {
                     ...
                     Невозможно выполнить запрос.Попробуйте обновить страницу или обратится к администратору.'
                        ...
                        if($num_rows =  mysql_num_rows($val2) != 0) {
                            $it2 = mysql_fetch_assoc($val2);
                            $i2 = $it2['CountPeopleWIthOpening'];
                        }else{
                            $i2 =0;
                        }
                    ...
                    if($i3 == 0)
                    {
                     ...
                     Невозможно выполнить запрос.Попробуйте обновить страницу или обратится к администратору.')));
                        if($num_rows =  mysql_num_rows($val3) != 0) {
                            $it3 = mysql_fetch_assoc($val3);
                            $i3 = $it3['CountPresentationWithOpening'];
                        }else{
                            $i3 =0;
                        }
                    ...
                    if($i4 == 0)
                    {
                     ...
                     Невозможно выполнить запрос.Попробуйте обновить страницу или обратится к администратору.')));
                        if($num_rows =  mysql_num_rows($val4) != 0) {
                            $it4 = mysql_fetch_assoc($val4);
                            $i4 = $it4['CountLettersWithOpening'];
                        }else{
                            $i4 =0;
                        }
